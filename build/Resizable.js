...
var Resizable = function (_React$Component) {
  ...
  Resizable.prototype.resizeHandler = function resizeHandler(handlerName, resizeAxis) {
    var _this2 = this;
    ...
    return function (e, _ref2) {
      ...
      // Axis restrictions
      var axis = resizeAxis || _this2.props.axis;

      var canDragX = ['both', 'x', 'xL', 'nw', 'ne', 'sw'].includes(axis); //axis === 'both' || axis === 'x';
      var canDragY = ['both', 'y', 'yT', 'nw', 'ne', 'sw'].includes(axis); //axis === 'both' || axis === 'y';
      ...
      if (hasCb) {
        ...
        _this2.setState(newState, function () {
          return _this2.props[handlerName](e, { node: node, size: { width: width, height: height }, axis: axis });
        });
      } else {
  ...
  Resizable.prototype.renderAxis = function renderAxis(axis) {
    var draggableOpts = this.props.draggableOpts;
    ...
    return _react2.default.createElement(
      ...
      _react2.default.createElement('span', { className: 'react-resizable-handle ' + (axis === 'both' ? '' : 'react-resizable-handle-' + axis) })
    );
  };
  ...
  Resizable.prototype.render = function render() {
    ...
    var _props = this.props,
        ...
        p = _objectWithoutProperties(_props, ['children', 'draggableOpts', 'width', 'height', 'handleSize', 'lockAspectRatio', 'resizer', 'axis', 'minConstraints', 'maxConstraints', 'onResize', 'onResizeStop', 'onResizeStart']);

    var className = p.className ? p.className + ' react-resizable' : 'react-resizable';

    var draggableCores = [];

    ['both', 'x', 'y', 'xL', 'yT', 'nw', 'sw', 'ne'].forEach(function (axisType) {
      if ((axis === 'both' || axisType === axis) && (resizer === 'all' || axisType === resizer)) {
        draggableCores.push(_this3.renderAxis(axisType));
      }
    });
    ...
    return (0, _cloneElement2.default)(children, _extends({}, p, {
      className: className,
      children: [children.props.children].concat(draggableCores)
    }));
  };
...
Resizable.propTypes = {
  ...
  resizer: _propTypes2.default.oneOf(['all', 'both', 'x', 'xL', 'y', 'yT', 'ne', 'nw', 'sw', 'none']),
