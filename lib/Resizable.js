...
type Resizer = Axis | 'xL' | 'yT' | 'ne' | 'nw' | 'sw' | 'all';
type State = {
  resizing: boolean,
...
export default class Resizable extends React.Component<Props, State> {
  static propTypes = {
    ...
    resizer: PropTypes.oneOf([ 'all', 'both', 'x', 'xL', 'y', 'yT', 'ne', 'nw', 'sw', 'none' ]),
  ...
  resizeHandler(handlerName: string, resizeAxis: Axis): Function {
    return (e: SyntheticEvent<> | MouseEvent, {node, deltaX, deltaY}: DragCallbackData) => {
      ...
      const canDragX = ['both', 'x', 'xL', 'nw', 'ne', 'sw'].includes(axis); //axis === 'both' || axis === 'x';
      const canDragY = ['both', 'y', 'yT', 'nw', 'ne', 'sw'].includes(axis); //axis === 'both' || axis === 'y';
  ...
  render(): ReactNode {
    ...
    ['both', 'x', 'y', 'xL', 'yT', 'nw', 'sw', 'ne'].forEach(axisType => {
      if (
